# app/scripts/GiveMeTitle/data_manager.py
...
class DataManager:
    ...
    def __init__(self):
        try:
            ...
            # 数据存储路径
            self.DATA_DIR = os.path.join(self.DATA_ROOT_DIR, "GiveMeTitle")
            # 群组开关数据目录
            ...
            logging.error(f"[GiveMeTitle]初始化数据管理器失败: {e}")
            raise
    ...
    def load_function_status(self, group_id):
        """查看功能开关状态"""
        try:
            return self.load_switch(group_id, "GiveMeTitle")
        except Exception as e:
            logging.error(f"[GiveMeTitle]加载功能状态失败, group_id={group_id}: {e}")
            return False  # 默认返回关闭状态

    def save_function_status(self, group_id, status):
        """保存功能开关状态"""
        try:
            self.save_switch(group_id, "GiveMeTitle", status)
            return True
        except Exception as e:
            logging.error(f"[GiveMeTitle]保存功能状态失败, group_id={group_id}: {e}")
            return False
    ...
    def is_authorized(self, user_id):
        ...
        try:
            ...
            logging.error(f"[GiveMeTitle]检查用户权限失败, user_id={user_id}: {e}")
    ...
    def save_data(self, filename, data):
        ...
        try:
            ...
            logging.error(f"[GiveMeTitle]保存数据失败 {filename}: {e}")
            return False
    ...
    def load_data(self, filename, default=None):
        ...
        try:
            ...
            logging.error(f"[GiveMeTitle]加载数据失败 {filename}: {e}")
            return default if default is not None else []
