# app/scripts/GiveMeTitle/private_message_handler.py
...
class PrivateMessageHandler:
    ...
    def __init__(self):
        ...
        try:
            ...
            logging.error(f"[GiveMeTitle]初始化私聊消息处理器失败: {e}")
            raise
    ...
    async def handle(self, msg):
        ...
        try:
            ...
            logging.error(f"[GiveMeTitle]处理私聊消息失败: {e}")
    ...
    async def _handle_friend_message(self):
        ...
        try:
            ...
            logging.error(f"[GiveMeTitle]处理好友私聊消息失败: {e}")
            if self.user_id:
                await send_private_msg(
                    self.websocket,
                    self.user_id,
    ...
    async def _handle_temp_message(self):
        ...
        try:
            ...
            logging.error(f"[GiveMeTitle]处理临时会话消息失败: {e}")
            if self.user_id:
                await send_private_msg(
                    self.websocket,
                    self.user_id,
    ...
    async def _handle_other_private_message(self):
        ...
        try:
            ...
            logging.error(f"[GiveMeTitle]处理其他来源私聊消息失败: {e}")
            if self.user_id:
                await send_private_msg(
                    self.websocket,
                    self.user_id,
