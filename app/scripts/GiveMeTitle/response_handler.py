# app/scripts/GiveMeTitle/response_handler.py
...
class ResponseHandler:
    ...
    def __init__(self):
        ...
        try:
            ...
            logging.error(f"[GiveMeTitle]初始化响应处理器失败: {e}")
            raise
    ...
    async def handle(self, msg):
        ...
        try:
            ...
            # 根据echo内容分发到不同的处理方法
            if self.echo.startswith("GiveMeTitle_group_"):
                await self.handle_group_operation_response(msg)
            elif self.echo.startswith("GiveMeTitle_private_"):
                await self.handle_private_operation_response(msg)
            elif self.echo.startswith("GiveMeTitle_request_"):
                await self.handle_request_operation_response(msg)
            elif self.echo.startswith("GiveMeTitle_notice_"):
                await self.handle_notice_operation_response(msg)
            elif self.echo.startswith("GiveMeTitle_scheduled_"):
                await self.handle_scheduled_task_response(msg)

        except Exception as e:
            logging.error(f"[GiveMeTitle]处理GiveMeTitle回调事件失败: {e}")
            ...
            if group_id:
                await send_group_msg(
                    self.websocket,
                    group_id,
                    f"处理GiveMeTitle回调事件失败，错误信息：{str(e)}",
                )
            elif user_id:
    ...
    async def handle_group_operation_response(self, msg):
        ...
        try:
            ...
            # 从echo中提取必要信息
            # 示例: GiveMeTitle_group_send_123456_987654321
            ...
            logging.error(f"[GiveMeTitle]处理群操作响应失败: {e}")
    ...
    async def handle_private_operation_response(self, msg):
        ...
        try:
            ...
            logging.error(f"[GiveMeTitle]处理私聊操作响应失败: {e}")
    ...
    async def handle_request_operation_response(self, msg):
        ...
        try:
            ...
            logging.error(f"[GiveMeTitle]处理请求操作响应失败: {e}")
    ...
    async def handle_notice_operation_response(self, msg):
        ...
        try:
            ...
            logging.error(f"[GiveMeTitle]处理通知操作响应失败: {e}")
    ...
    async def handle_scheduled_task_response(self, msg):
        ...
        try:
            ...
            logging.error(f"[GiveMeTitle]处理定时任务响应失败: {e}")
